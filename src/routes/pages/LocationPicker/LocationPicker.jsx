...
export default function LocationPicker() {
    ...
    const {
        itemSummary, totalAfterDiscount, showInput, setShowInput,
        address, serviceTitle, setMapLongitude, setMapLatitude,
        setAddressLocation, liveAddress, saveAddress, setLiveAddress,
        totalVatRate, mapLongitude, mapLatitude
    } = useSummary();

    const [selectedAddressId, setSelectedAddressId] = useState(liveAddress?.id || null);
                        ...
                        <div className="relative">
                            ...
                            <div className="relative">
                                ...
                                <div className="absolute top-3 right-3 z-20 flex flex-col gap-1.5">
                                    {[
                                        { icon: <FaPlus className="w-3 h-3" />, action: () => map?.setZoom(map.getZoom() + 1), title: "Zoom in" },
                                        { icon: <FaMinus className="w-3 h-3" />, action: () => map?.setZoom(map.getZoom() - 1), title: "Zoom out" },
                                        { icon: <FaLocationCrosshairs className="w-3 h-3" />, action: gotoMyLocation, title: "My location" },
                                        { icon: <FaSatellite className="w-3 h-3" />, action: () => setMapType(mapType === "roadmap" ? "hybrid" : "roadmap"), title: "Toggle satellite" },
                                    ...
                                    <div className="absolute inset-0 z-30 bg-black/50 backdrop-blur-[2px] flex items-center justify-center">
                                        <div className="bg-white rounded-2xl p-6 w-[85%] max-w-xs text-center shadow-2xl">
                                            ...
                                            <p className="text-xs text-gray-500 mb-5 leading-relaxed">
                                                We can automatically detect your location to make address entry faster.
                                            </p>
